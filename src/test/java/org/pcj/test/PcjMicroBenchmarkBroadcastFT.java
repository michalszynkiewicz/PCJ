...
@RegisterStorage(PcjMicroBenchmarkBroadcastFT.SharedEnum.class)
public class PcjMicroBenchmarkBroadcastFT implements StartPoint {

    @Storage(PcjMicroBenchmarkBroadcastFT.class)
    enum SharedEnum {
        a
    }
    double[] a;
    ...
    public void main() {
        ...
        for (int n : transmit) {
            ...
            PCJ.barrier();
            System.out.println("(" + PCJ.getNodeId() + "] after barrier before node out");
            if (PCJ.getNodeId() == 2) {
                System.exit(1);
            }
            ...
            PCJ.barrier();
            System.out.println("(" + PCJ.getNodeId() + "] after barrier after node out");
...
//        Level level = Level.FINEST;
        ...
        //        int[] threads = {1, 4, 16};
        int maxNodes = 10;
//        int[] threads = {6}; mstodo bring back
        int[] threads = {1};
        // run.jvmargs=-Xmx64m
...
//        for (int m = n_nodes; m > 0; m = m / 2) 
        int m = n_nodes;
        {
            ...
            for (int nt : threads) {
                ...
                PCJ.deploy(PcjMicroBenchmarkBroadcastFT.class,
